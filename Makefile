...
EXE_PREFIX = /opt/miyoo-toolchain/bin/arm-buildroot-linux-musleabi-
CC        = ${EXE_PREFIX}gcc
STRIP     = ${EXE_PREFIX}strip
LD = $(CC)

CFLAGS = -O3 `/opt/miyoo-toolchain/arm-buildroot-linux-musleabi/sysroot/usr/bin/sdl-config --cflags` 
LDFLAGS = -s

LIBS = -lc `/opt/miyoo-toolchain/arm-buildroot-linux-musleabi/sysroot/usr/bin/sdl-config --libs` 

CFLAGS += -DDINGOO_BUILD -DDINGOO_OPENDINGUX -DDISABLE_ROMBROWSER
